...
<div class="flex flex-col md:flex-row justify-between items-end md:items-center mb-10 gap-4">
  <div>
    <h1 class="text-4xl font-extrabold text-white tracking-tight">Búsqueda de Referencias</h1>
    <p class="text-slate-400 mt-1">Gestión de Referencias ICO</p>
  </div>
  <button onclick="toggleModal('modal-create')" class="group relative px-6 py-3 font-bold text-white transition-all duration-300">
    <div class="absolute inset-0 bg-blue-600 rounded-xl blur-md opacity-20 group-hover:opacity-40 transition duration-300"></div>
    <div class="relative bg-blue-600 hover:bg-blue-500 px-6 py-3 rounded-xl transition-all border border-blue-500/50 shadow-[0_0_15px_rgba(37,99,235,0.3)]">
      + Nueva Referencia
    </div>
  </button>
</div>
...
<div class="relative group">
  <div class="absolute -inset-1 bg-gradient-to-r from-blue-600 to-purple-600 rounded-2xl blur opacity-10 group-hover:opacity-20 transition duration-1000"></div>
  ...
  <div class="relative bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl shadow-2xl overflow-hidden flex flex-col">
    ...
    <div class="overflow-x-auto">
      <table class="w-full text-left border-collapse">
        <thead>
          <tr class="border-b border-white/10 text-slate-400 text-sm uppercase tracking-wider">
            <th class="p-5 font-medium">Referencia (Formato APA Preview)</th>
            <th class="p-5 font-medium">Clasificación</th>
            ...
            <td class="p-5 max-w-xl">
              <div class="leading-relaxed">
                ...
                <span class="text-slate-300">
                  @if($ref->volumen) Vol. {{ $ref->volumen }}, @endif
                  @if($ref->numero) No. {{ $ref->numero }}, @endif
                  @if($ref->paginas) pp. {{ $ref->paginas }}. @endif
                  @if($ref->editorial) {{ $ref->editorial->nombre }}. @endif
                </span>

                @if($ref->doi || $ref->url)
                <br><span class="text-blue-400/80 text-[10px] break-all">{{ $ref->doi ? 'https://doi.org/'.$ref->doi : $ref->url }}</span>
                @endif
              </div>
            </td>
            <td class="p-5 align-top">
              <span class="inline-block bg-blue-500/20 text-blue-300 px-3 py-1 rounded-full border border-blue-500/30 text-xs mb-2 uppercase">
                {{ $ref->tipo_referencia->nombre ?? 'N/A' }}
              </span>
              <div class="flex flex-wrap gap-1">
                @foreach($ref->materias as $materia)
                <span class="bg-black/50 border border-white/10 text-slate-400 px-2 py-0.5 rounded text-[10px] uppercase">{{ $materia->nombre }}</span>
                @endforeach
              </div>
            </td>
    ...
    <div class="p-5 border-t border-white/10 bg-black/20 mt-auto">
      {{ $referencias->links() }}
    </div>
